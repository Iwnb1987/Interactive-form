  //Adding input types to manipulate later
...
bluePrint.addEventListener('change', (e) => {
 ...
 if(dataShade === dataColor) {                                      //It'll check if lines 33 and 34 variables match
...
//this change will display the "i'm going to pay with" drop down menu
...
forBox[i].addEventListener('blur', (e) => {
  ...
  forBox[i].parentElement.classList.remove('focus');
     ...
     if(e.target.type ==='checkbox') {
        ...
        //And prevents selecting activities that match
        for(let i = 0; i < forBox.length; i++) {
